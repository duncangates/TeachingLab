...
table_maker_legacy_3 <- function(data, column_names, title, spanner, note1, note2, note3, note4, save_name, rows_positive, rows_negative) {
  ...
  sy1920_report <- data %>%
    ...
    tab_source_note(
      source_note = html("<i>**This could not be calculated because the four ELA educators who completed the follow-up survey had not taken the diagnostic survey.</i>")
    ) %>%
    # Final theming
    gt_theme_tl() %>%
    tab_options(
