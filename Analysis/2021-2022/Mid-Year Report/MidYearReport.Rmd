...
params:
  matched: "unmatched"
  partner: "Freire Charter Schools, PA/DE"
...
special_reports <- c("All Partners", "District 11", "District 11 Math", "District 11 ELA",
                     "San Diego K-5", "San Diego 6-8", "NYC District 12 - EMST-IS 190, NY", 
                     "Fannie Lou", "Amistad")
...
# knowledge_assessments <- list.files(here::here("Dashboards/KnowledgeAssessments/data/processed"), 
#                                     full.names = T) %>%
#   filter(site == params$site)
knowledge_assessments <- tibble(x = 1:907)
...
- `r nrow(session_survey)` responses to the End of Session Surveys that gather participant feedback on facilitation from session to session;

- `r nrow(course_survey)` responses to the End of Course Surveys that gather participant feedback on each PL course;

- `r nrow(knowledge_assessments)` responses to the Knowledge or Self-Reported Practices Assessments the first and last day of each PL course;

- `r nrow(diagnostic)` responses to the Baseline Diagnostic Educator Survey that collects information on teachers’ use of curricula, mindsets, self-reported practices, and school environment. The follow-up will be administered in the spring after a year of PL engagement.
...
<div class = "fold o">
```{r gender-identity, fig.dim = c(5, 6)}
...
<div class = "fold o">
```{r racial-identity, fig.dim = c(5, 6)}
if (diagnostic %>%
...
<div class = "fold o">
```{r racial-identity2, fig.dim = c(5, 6)}
...
<div class = "fold o">
```{r prior-experience, fig.dim = c(5, 6)}
...
ggplot(data = plot_agree, aes(x = Question, y = Percent, fill = factor(Response))) +
        ...
        labs(
          fill = "", title = "Participant Perceptions of Course Facilitation",
          x = "", y = ""
        ) +
        coord_flip() +
...
<div class = "fold o">
```{r facilitation-feedback, fig.dim = c(5, 6)}
...
<div class = "fold o">
```{r course-quote-1, fig.dim = c(5, 6)}
...
<div class = "fold o">
```{r course-quote-2, fig.dim = c(5, 6)}
...
<div class = "fold o">
```{r course-quote-3, fig.dim = c(5, 6)}
...
<div class = "fold o">
```{r course-quote-4, fig.dim = c(5, 6)}
...
<div class = "fold o">
```{r course-quote-5, fig.dim = c(5, 6)}
...
<div class = "fold o">
```{r use-of-curricula, fig.width = 12, fig.height = 12}
...
<div class = "fold o">
```{r teacher-mindsets, fig.width = 11, fig.height = 14}
  ...
  plot_data %>%
    ggplot(aes(x = value, y = percent)) +
              ...
              labs(y = "Percent", x = "Response",
                   ...
                   subtitle = glue::glue("<i>Average score: {teacher_mindset_average}/5, n = {prettyNum(sum(plot_data$n, na.rm = T), big.mark = ',')}</i>")) +
              scale_x_discrete() +
              scale_y_continuous(labels = scales::percent_format(scale = 1), 
...
<div class = "fold o">
```{r teacher-crse, fig.width = 10, fig.height = 14}
if (sum(!is.na(diagnostic$please_rate_your_confidence_on_the_following_items_br_i_am_able_to_adapt_instruction_to_meet_the_needs_of_my_students))) {
  ...
  plot_data %>%
    ggplot(aes(x = value, y = percent)) +
              ...
              labs(y = "Percent", x = "Response",
                   title = "To what extent do you agree or disagree<br>with the following statements",
                   subtitle = glue::glue("Average score: {teacher_crse_average}/10, n = {prettyNum(sum(plot_data$n, na.rm = T), big.mark = ',')}")) +
              scale_x_discrete(breaks = as.character(c(1:10))) +
              scale_y_continuous(labels = scales::percent_format(scale = 1), 
                                 breaks = scales::pretty_breaks(n = 4)) +
    scale_color_manual(values = c("white", "black")) +
    theme_tl() +
...
<div class = "fold o">
```{r teacher-school-environment, fig.width = 11, fig.height = 14}
  ...
  plot_data %>%
    ggplot(aes(x = value, y = percent)) +
              ...
              labs(y = "Percent", x = "Response",
                   ...
                   subtitle = glue::glue("Average score: {teacher_school_environment_average}/5, n = {prettyNum(sum(plot_data$n, na.rm = T), big.mark = ',')}")) +
              scale_x_discrete() +
              scale_y_continuous(labels = scales::percent_format(scale = 1), 
                                 breaks = scales::pretty_breaks(n = 4)) +
    scale_color_manual(values = c("white", "black")) +
...
<div class = "fold o">
```{r administrator-mindsets, fig.width = 12, fig.height = 14}
if (sum(!is.na(diagnostic$to_what_extent_do_you_agree_or_disagree_with_the_following_statements_teachers_should_be_color_blind_when_it_comes_to_their_teaching_they_shouldnt_think_of_students_in_terms_of_their_race_or_ethnicity))) {
  ...
  plot_data %>%
    ggplot(aes(x = value, y = percent)) +
              ...
              labs(y = "Percent", x = "Response",
                   title = "To what extent do you agree or<br>disagree with the following statements",
                   subtitle = glue::glue("Average score: {administrator_mindsets_average}/5, n = {prettyNum(sum(plot_data$n, na.rm = T), big.mark = ',')}")) +
              scale_x_discrete() +
              scale_y_continuous(labels = scales::percent_format(scale = 1), 
                                 breaks = scales::pretty_breaks(n = 4)) +
    scale_color_manual(values = c("black", "white")) +
...
<div class = "fold o">
```{r administrator-support, fig.width = 10, fig.height = 12}
if (sum(!is.na(diagnostic$please_rate_the_extent_to_which_you_believe_you_can_support_teachers_in_the_following_areas_br_i_believe_i_can_directly_or_indirectly_support_teachers_to_adapt_instruction_to_meet_the_needs_of_their_students))) {
  ...
  plot_data <- diagnostic %>%
                  ...
                  mutate(name = html_wrap(str_to_title(str_replace_all(name, "_", " ")), n = 30)) %>%
                  tidyr::drop_na(value) %>%
                  group_by(value, name) %>% # Group by both name and value
                  summarise(n = n()) %>% # Get n of union of subgroup
                  group_by(name) %>% # Group by just value
  ...
  plot_data %>%
    ggplot(aes(x = value, y = percent)) +
              ...
              labs(y = "Percent", x = "Response",
                   title = "To what extent do you agree or<br>disagree with the following statements?",
                   subtitle = glue::glue("Average score: {administrator_support_crt_average}/10, n = {prettyNum(sum(plot_data$n, na.rm = T), big.mark = ',')}")) +
              scale_x_discrete() +
              scale_y_continuous(labels = scales::percent_format(scale = 1), 
                                 breaks = scales::pretty_breaks(n = 4)) +
    scale_color_manual(values = c("white", "black")) +
...
<div class = "fold o">
```{r administrator-observational, fig.width = 10, fig.height = 12}
if (sum(!is.na(diagnostic$how_often_does_your_observation_of_teacher_practice_focus_on_the_following_whether_the_lesson_is_focused_on_a_high_quality_text_or_task))) {
  ...
  plot_data %>%
    ggplot(aes(x = value, y = percent)) +
              geom_col(aes(fill = percent)) +
              geom_richtext(aes(label = paste0("**", round(percent), "%**")), vjust = -0.1,
                        size = 6, lineheight = 0.5, fill = NA, label.color = NA, color = "black") +
              facet_wrap( ~ name, ncol = 3, scales = "free_x") +
              labs(y = "Percent", x = "Response",
                   title = "How often does your observation of teacher <br>practice focus on the following",
                   subtitle = glue::glue("Average score: {administrator_observational_average}/5, n = {prettyNum(sum(plot_data$n, na.rm = T), big.mark = ',')}")) +
              scale_x_discrete() +
