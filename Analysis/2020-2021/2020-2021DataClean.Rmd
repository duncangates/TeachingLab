...
fall_df_compare <- bind_cols(fall_vars, fall_cols) %>%
  ...
  bind_rows(tibble(class = rep(NA, 9), value = rep(NA, 9), name = rep(NA, 9))) %>%
  select(-name)
...
fall_one <- fall_df %>% filter(id %in% matched_filter)
spring_one <- spring_df %>% filter(id %in% matched_filter)

matched_2021 <- bind_cols(fall_one %>% arrange(id), spring_one %>% arrange(id) %>% select(-id))

write_rds(matched_2021, here("Analysis/2020-2021/Data/matched_2021.rds"))
```
