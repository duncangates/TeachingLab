...
title: "`r paste0('Teaching Lab All Partners Report: ', stringr::str_to_title(params$matched))`"
...
table_maker(data = environment_gt, 
            column_names = colnames(data)[2:5], 
...
ggplot() +
    ...
    scale_y_discrete(expand = c(0, 1)) +
    labs(x = NULL, y = NULL,
         title = "Educators' Average Scores on ELA Content and Pedagogical <br> Content Knowledge in the <span style = 'color:#ff7b43;'>Diagnostic</span> and <span style = 'color:#00acf0;'>Follow-up<span> Surveys") +
    theme_bw() +
    theme(panel.border = element_blank(),
          legend.position = "none",
          plot.title = element_markdown(hjust = 0.5, family = "Calibri", size = 20, lineheight = 1.15),
...
admin_gt <- tibble(
  rowname = c("When observing teachers, I focus on… \U2001 \U2001 \U2001 \U2001 \U2001 Overall score", 
...
ggplot() +
    ...
    labs(x = NULL, y = NULL,
         title = "Shifts in Observation Practices of Coaches, Leaders, and Administrators from the <br> <span style = 'color:#ff7b43;'>Diagnostic</span> and <span style = 'color:#00acf0;'>Follow-up<span> Surveys") +
    theme_bw() +
    theme(panel.border = element_blank(),
          legend.position = "none",
          plot.title = element_markdown(hjust = 0.5, family = "Calibri", size = 20, lineheight = 1.15),
...
teacher <- pmap_df(list(teacher_index$question_pre, teacher_index$question_post, teacher_index$coding), 
        ~ score_question_improved(data, question_pre = ..1, question_post = ..2, 
...
teacher %>%
  ...
  tab_header(html("<strong>Differences in Teaching Practices between Teaching Lab Participants and Non-participants, as Reported by Coaches, Leaders, and Administrators")) %>%
  fmt_percent(
    columns = c(1, 2),
    scale_values = F,
    decimals = 0
  ...
  tab_footnote(footnote = glue::glue("n = {n1_teacher}"),
               locations = cells_column_labels(
                 columns = c(1)
               )) %>%
  tab_footnote(footnote = glue::glue("n = {n2_teacher}"),
               locations = cells_column_labels(
                 columns = c(2)
               )) %>%
  gt_theme_tl()
...
teacher_graph <- teacher %>%
  ...
  mutate(question = str_replace_all(str_wrap(question, 20), "\n", "<br>"))
...
ggplot(teacher_graph) +
  ...
  labs(x = "",
       y = "",
       title = "Comparison of Administrators Reviews of <br> <span style = 'color:#00acf0;'>Teaching Lab</span> and <span style = 'color:#000000;'>Non-Teaching Lab<span> Teachers",
       fill = "") +
  scale_y_continuous(labels = percent_format(scale = 1), expand = c(0.1, 0)) +
  ...
  theme_tl() +
  theme(axis.text.x = element_markdown(lineheight = 1.1),
...
lab_replace <- c("Improve my" = "Improved their",
                 "Lead" = "Led",
                 "Coach" = "Coached",
                 "Share" = "Shared")
...
lableader_df <- data %>%
  ...
  mutate(post_ll4a = str_replace_all(post_ll4a, lab_replace))


if (nrow(lableader_df) > 0) {
  lableader_df %>%
    gt(rowname_col = "post_ll4a") %>%
    cols_label(
      `n()/lableaders` = html("<strong><span style = 'color:#00acf0;'>% of Lab Leaders who engaged in the activity")
    ) %>%
    fmt_percent(
      columns = c(2),
      decimals = 0
...
`r if_else(params$partner %!in% c("All Partners"), glue::glue("No student work was analyzed at this location."), glue::glue("Student work has been analyzed to see if there was a difference in outcomes following the implementation of professional learning. The following figures illustrate that difference."))`


```{r, results = "asis"}
files <- list.files(here("Images/2020-2021/StudentWork"), full.names = T)[-4]

if (params$partner %in% c("All Partners")) {
  knitr::include_graphics("/Users/dunk/Teaching Lab/Coding/TeachingLab/Images/2020-2021/StudentWork/Table1.png")
}
...
```{r}
if (params$partner != "All Partners") {
  dashboard_data <- read_rds(here::here("Data/Dashboard Data/dashboard_data.rds")) %>%
    filter(`District, Parish, Or Network` == params$partner) %>%
...
dashboard_gt %>%
  gt() %>%
  tab_header(
    html(glue::glue("Teaching Lab Performance on Participant Feedback Questions for {params$partner}"))
  ) %>%
  cols_label(
...
gained <- dashboard_data %>%
  ...
  filter(!is.na(`Overall, what went well in this professional learning?`) & 
           ...
           str_length(`Overall, what went well in this professional learning?`) > 80) %>%
  slice_sample(n = 10) %>%
  distinct()
...
improved_experience <- dashboard_data %>%
  ...
  filter(!is.na(`What could have improved your experience?`) & 
           ...
           str_length(`What could have improved your experience?`) > 80) %>%
  slice_sample(n = 10) %>%
  distinct()
...
add_comments <- dashboard_data %>%
  ...
  filter(!is.na(`Do you have additional comments?`) & 
           ...
           str_length(`Do you have additional comments?`) > 80) %>%
  slice_sample(n = 10) %>%
  distinct()
```
...
```{r} 
TeachingLab::quote_viz(add_comments, `Do you have additional comments?`, "gt")
```
