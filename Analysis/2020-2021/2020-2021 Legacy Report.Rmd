...
score_question2 <- function(data, question, coding) {
...
unmatched_mindsets_pre <- map2_df(index$question, index$coding, ~ score_question2(data = fall_legacy, question = .x, coding = .y)) %>%
  mutate(final_score = score / (n*2))
# Matched
matched_mindsets_pre <- map2_df(index$question, index$coding, ~ score_question2(data = fall_legacy %>% filter(id %in% legacy_matched_filter & pre_role == "Teacher"), question = .x, coding = .y)) %>%
  mutate(final_score = score / (n*2))
...
unmatched_mindsets_post <- map2_df(index$question, index$coding, ~ score_question2(data = spring_legacy, question = .x, coding = .y)) %>%
  mutate(final_score = score / (n*2))
# Matched
matched_mindsets_post <- map2_df(index$question, index$coding, ~ score_question2(data = spring_legacy %>% filter(id %in% legacy_matched_filter), question = .x, coding = .y)) %>%
  mutate(final_score = score / (n*2))
...
#   # pivot_longer(!c(score_pre, score_post, n1, n2)) %>%
#   # pivot_wider(names_from = "name")
#   mutate(id = row_number()) %>%
#   relocate(id, .before = 1) %>%
#   select(1, 2,3,4,5) %>% 
#   group_by(id) %>%
#   summarise(score_pre = sum(score_prerace1, score_prerace2),
#             score_post = sum(score_postrace1, score_postrace2)) %>%
#   mutate(improve = if_else(score_post >= score_pre & score_post != 0, T, F)) %>%
#   summarise(improvement = sum(improve)/length(improve))
...
unmatched_mindsets_pre <- pmap_dfc(list(index$question_pre, index$question_post, index$coding),
  ...
  mutate(id = row_number()) %>%
  relocate(id, .before = 1) %>%
  # # dplyr::select(id, starts_with("score")) %>%
