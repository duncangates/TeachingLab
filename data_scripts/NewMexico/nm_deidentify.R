...
extra_names_and_emails <- names_and_emails %>%
  ...
  select(`Work email`) %>%
  drop_na(`Work email`) %>%
  bind_cols(tibble::tibble(`Which of the following best describes your primary role? <br><br>` = c("Teacher",
                                                                                                   "Administrator",
                                                                                                   "Administrator",
                                                                                                   "Administrator",
                                                                                                   "Administrator",
...
## Get stored ids from previous run ##
# stored_ids <- readr::read_rds(here::here("data/new_mexico_data/stored_ids.rds"))
...
canvas_videos_renamed <- canvas_videos %>%
  mutate(user.name = str_to_title(user.name),
         user.name = case_when(
           ...
           str_detect(str_to_lower(user.name), 
                      "adrian") == T ~ "Adrian Apodaca",
           str_detect(str_to_lower(user.name), 
                      "jamie hephner") == T ~ "Jamie Lopez",
...
videos <- tibble::tibble(number = c(17, 7, 5, 12, 13, 4, 11, 1, 6, 15, 14, 10, 9, 3),
                         video_link = c("https://youtu.be/3S5AMuR1aKA",
                                        ...
                                        list(c("https://youtu.be/v-KLlMXrzFQ", "https://youtu.be/PQBrp4W8ZrI")),
                                        "https://youtu.be/Kf6SvnHYQ-U",
                                        "https://youtu.be/DylN6zfayNo",
                                        "https://youtu.be/z55SNXGMA44",
                                        "https://youtu.be/1nFWt1t2BLQ",
                                        "https://youtu.be/I4xvS3ZEH6g")) %>%
  arrange(number)
...
## Upload to dropbox
drop_upload(file = here::here("data/new_mexico_data/new_mexico_21_22_deidentified.csv"),
...
## Upload to dropbox
drop_upload(file = here::here("data/new_mexico_data/student_new_mexico_21_22_deidentified.csv"),
            path = "/New Mexico Deidentified Data (2021-2022)",
            mode = "overwrite")
