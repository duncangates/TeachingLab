...
test %>%
  drop_na(question) %>%
          ...
          # ungroup() %>%
          ggplot2::ggplot(aes(x = question, y = n, fill = n)) + # Pie chart input, ordered by n
          labs(
...
profile <- get_user_profile("abconcepcion.99")
playlists <- get_user_playlists("abconcepcion.99")
albums <- get_artist_albums("7pbDxGE6nQSZVfiFdq9lOL")
```
