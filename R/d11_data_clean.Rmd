...
data <- read_csv(here("Data/D11 CRT PLC End of Course Survey 2.csv"), skip = 1)
...
fall_spring <- bind_rows(
  ...
  after_data %>%
    rename_with(~ str_remove(., " - After the PLC"))
) %>%
  mutate(season = c(rep("Pre PLC", 23), rep("Post PLC", 23))) %>%
  group_by(season) %>%
...
ggplot() +
  ...
  labs(
    x = NULL, y = NULL,
    title = "Piloted CRT Items Using Retrospective <span style = 'color:#00acf0;'>Pre</span>/<span style = 'color:#ff7b43;'>Post</span> Design"
  ) +
  theme_bw() +
  theme(
    panel.border = element_blank(),
...
ggsave(here("Images/D11Beliefs.png"), width = 12, height = 8)
```
...
y_vector <- c(0.6, 0.15, 0.6, 0.15, 0.6, 0.1, 0.025)
...
ggplot(question6, aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = `Time Length`)) +
  geom_rect(color = "black", size = 0.25) +
  geom_text(x = 1.9, aes(y = label_pos, label = label, color = `Time Length`), size = 5) + # x here controls label position (inner / outer)
  scale_fill_manual(values = rev(col(3))) +
  scale_color_manual(values = rev(col(3))) +
  labs(title = "Most thought there was <span style = 'color:#04ABEB;'>just enough</span> time, but some<br> also thought there was <span style = 'color:#045777;'>not enough</span> time") +
  coord_polar(theta = "y") +
  xlim(c(-1, 4)) +
  theme_void() +
...
question7$y <- c(0.73, 0.33, 0.17, 0.0725)
...
ggplot(question7) +
  ...
  labs(x = NULL, y = NULL, title = "Participants are ideally willing to engage in <span style = 'color:#04ABEB;'>0-30 mins</span> per session") +
  guides(color = F, fill = guide_legend(reverse = T)) +
  scale_color_manual(values = c("FALSE" = "black", "TRUE" = "white")) +
  scale_x_discrete() +
  scale_y_continuous(labels = scales::percent) +
...
ggplot(item10) +
  geom_col(aes(x = fct_reorder(Question, percent), y = `percent`, fill = `Understanding`), color = "black", size = 0.18) +
  geom_text(aes(x = fct_reorder(Question, percent), y = percent, 
                label = if_else(percent > 0.1, paste0(round(100 * percent, 2), "%"), "")),
                position = position_stack(vjust = 0.5), 
            family = "Fira Sans") +
  labs(x = NULL, y = NULL, title = "Understanding of CRT Varies by Question") +
