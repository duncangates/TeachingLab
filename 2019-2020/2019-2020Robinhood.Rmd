...
mindsets_segments <- tibble(
  ...
  text_fall = c(61,91,37,54),
  text_spring = c(65,95,56,46),
  size1 = c(50, 40, 50, 50),
  size2 = c(50, 40, 50, 50)
)
...
ggplot() +
  geom_point(data = mindsets, mapping = aes(color = season, x = ed, y = obs, size = ed)) +
...
ggplot() +
  ...
  scale_x_continuous(labels = scales::percent_format(scale = 1), breaks = scales::pretty_breaks(n = 5)) +
  labs(x = NULL, y = NULL,
       title = "Increase in Trust and Connectedness in Educators from <span style = 'color:#ff7b43;'>Fall</span> to <span style = 'color:#00acf0;'>Spring</span>") +
  theme_bw() +
  theme(panel.border = element_blank(),
        legend.position = "none",
        plot.title = element_markdown(hjust = 0.5, family = "Fira Sans", size = 20),
...
ela_content_segments <- tibble(
  ...
  text_fall = c(59,47,50,49,33,78),
  text_spring = c(68,53,40,62,56,78),
)
...
ggplot() +
  ...
  scale_x_continuous(labels = scales::percent_format(scale = 1), breaks = scales::pretty_breaks(n = 5)) +
  labs(x = NULL, y = NULL,
       title = "Educators' Averages Scores on Content and<br>Pedagogical Content Knowledge in <span style = 'color:#ff7b43;'>Fall</span> and <span style = 'color:#00acf0;'>Spring</span>") +
  theme_bw() +
  theme(panel.border = element_blank(),
        legend.position = "none",
        plot.title = element_markdown(hjust = 0.5, family = "Fira Sans", size = 20),
...
effect_size <- function(percent1, percent2) {
  size1 <- round((percent1*19))
  size1_inv <- 19-size1
  size2 <- round((percent2*19))
  size2_inv <- 19-size2
