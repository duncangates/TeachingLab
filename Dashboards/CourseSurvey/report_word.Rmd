...
img1_loc <- tlShiny::file.path2(tempdir(), "img1.png")
...
```{r fig.width = 8, fig.show = "hide"}
img2_loc <- tlShiny::file.path2(tempdir(), "img2.png")
...
```{r fig.width = 8, fig.show = "hide"}
img3_loc <- tlShiny::file.path2(tempdir(), "img3.png")
...
```{r fig.width = 8, fig.show = "hide"}
img4_loc <- tlShiny::file.path2(tempdir(), "img4.png")
if (nrow(as_tibble(params$quote4)) >= 1) { 
  ...
  gtsave(filename = img4_loc)
} else {
  img4 <- tibble("There were no comments to display here.") %>% 
    quote_viz(title = "Nothing", 
              width = 40,
              print = F) %>% 
    gtsave(filename = img4_loc)
...
if (nrow(as_tibble(params$quote5)) >= 1) { 
  ...
  gtsave(filename = img5_loc)
} else {
  img5 <- tibble("There were no comments to display here.") %>% 
    quote_viz(title = "Nothing", 
              width = 40,
              print = F) %>% 
    gtsave(filename = img5_loc)
...
```{r}
knitr::include_graphics(img5_loc)
```
