...
img1_loc <- file.path2(tempdir(), "img1.png")
if (nrow(as_tibble(params$quote1)) >= 1) { 
  img1 <- quote_viz(
    ...
    gtsave(filename = img1_loc)
} else {
  img1 <- tibble("There were no comments to display here.") %>% 
...
```{r}
knitr::include_graphics(img1_loc)
```
...
```{r fig.width = 8, fig.show = "hide"}
img2_loc <- file.path2(tempdir(), "img2.png")
if (nrow(as_tibble(params$quote2)) >= 1) { 
  img2 <- quote_viz(
    ...
    gtsave(filename = img2_loc)
} else {
  img2 <- tibble("There were no comments to display here.") %>% 
    quote_viz(title = "Nothing", 
              width = 40,
              print = F) %>% 
    gtsave(filename = img2_loc)
...
if (nrow(as_tibble(params$quote3)) >= 1) { 
  img3 <- quote_viz(
    ...
    gtsave(filename = img3_loc)
} else {
  img3 <- tibble("There were no comments to display here.") %>% 
    quote_viz(title = "Nothing", width = 40,
              print = F) %>% 
    gtsave(filename = img3_loc)
}
```
...
```{r}
knitr::include_graphics(img3_loc)
```
